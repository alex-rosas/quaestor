...
The live pipeline requires Ollama (embeddings) and a Groq API key (LLM). To let anyone explore real system behaviour without setup, there is a **static demo** that replays pre-computed responses from the 20-question golden evaluation dataset:

```bash
uv run streamlit run app_static.py
```

No backend needed. All responses are real — generated by actual pipeline runs on the Apple FY2025 10-K and stored in `eval/results/`. Switch between **Phase 2** (child-only, high refusal rate) and **Phase 3** (parent injection, refusals eliminated) in the sidebar to see the architectural difference directly.

The live interactive app (requires Ollama + Groq key + indexed documents):
